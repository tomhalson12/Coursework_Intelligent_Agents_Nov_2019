...
+! plan : true <-
		...
		+ max_speed(S);
		+ max_scan(R);
		actions.calc_searches_to_do(W, H, 4, 5, N);
		+ searches_to_do(N);
		actions.calc_start_point(W, H, 4, 1, S_X, S_Y);
...
+! choose_task : diamond(X, Y, _) & not collecting_res(_, _) & not diamond_being_collected(X, Y) <-
		- free;
		! handle_res(X, Y, Q).
...
-! searching : movement_obstructed(_,_,_,_) <-
...
+ found_diamond(X, Y, Q) : diamond(X, Y, _) <-
...
@resource[atomic]
+ resource_found("Diamond", Q, X, Y) : true <-
		! count_others;
...
+ resource_found(T, Q, X, Y) : T \== "Diamond" <-
		! count_others;
		actions.convert_distance(X, Y, X2, Y2);
		? last_scan(SCAN_X, SCAN_Y);
...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & diamond(X, Y, Q) & not movement_obs & not diamond_being_collected(X, Y) <-
		- searching;
		+ collecting_res(X, Y);
		.send(scen4diamondLeader, tell, diamond_being_collected(X, Y));
		!! move_to_res(X, Y).

+! handle_res(X, Y, Q) : not collecting_res(_, _) & not diamond(X, Y, _) & not movement_obs & not diamond_being_collected(X, Y) <-
		+ diamond(X, Y, Q);
		+ searching;
		+ collecting_res(X, Y);
		.broadcast(tell, found_diamond(X, Y, Q));
...
-! move_to_res(X, Y) : movement_obstructed(_,_,_,_) <-
		.print('Moving to res failed - nagivating obstacle').
		
-! move_to_res(X, Y) : true <-
		+ free.
		
+! pickup_res : collecting_res(X, Y) & diamond(X, Y, 0) <-
		- diamond(X, Y, 0);
		- collecting;
		.send(scen4diamondLeader, tell, picked_diamond(X, Y));
		!! deposit_load.
...
+! pickup_res : collecting_res(X, Y) & diamond(X, Y, Q) <-
		collect(G).

-! pickup_res : true <-
		- collecting.
		
+ action_completed(collect) : collecting_res(X, Y) & diamond(X, Y, Q) & Q > 0 <-
...
-! return_to_base : movement_obstructed(_,_,_,_) <-
...
+! deposit_res : carrying(0) <-
		- collecting_res(X, Y);
		.send(scen4diamondLeader, tell, collected_diamond(X, Y));
		- depositing;
		+ free.
...
+! count_others : not others(_) & .count(resource_found("Gold",_, _,_), D) & .count(resource_found("Obstacle", _, _, _), O) & D + O > 0 <-
		+ others(D + O).
		
+! count_others : true <-
		? max_scan(R).
		
+! handle_other(T, Q, X, Y) : T == "Gold" <-
		- others(O);
		+ others(O - 1);
...
+! choose_action : collecting_res(_, _) | .count(resource_found("Diamond", _, _, _) ,C) & C > 0 <-
		- others(_).
...
-! move_to_target : movement_obstructed(_,_,_,_) <-
...
-! find_new_scan_loc : movement_obstructed(_,_,_,_) <-
