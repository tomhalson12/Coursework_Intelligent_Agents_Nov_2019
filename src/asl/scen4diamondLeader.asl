...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & diamond(X, Y, Q) & not movement_obs & not diamond_being_collected(X, Y) <-
		- searching;
		+ collecting_res(X, Y);
		.send(scen4diamondWorker, tell, diamond_being_collected(X, Y));
		!! move_to_res(X, Y).
...
+! pickup_res : collecting_res(X, Y) & diamond(X, Y, 0) <-
		- diamond(X, Y, 0);
		- collecting;
		.send(scen4diamondWorker, tell, picked_diamond(X, Y));
		!! deposit_load.
...
+! deposit_res : carrying(0) <-
		- collecting_res(X, Y);
		.send(scen4diamondWorker, tell, collected_diamond(X, Y));
		- depositing;
		+ free.
