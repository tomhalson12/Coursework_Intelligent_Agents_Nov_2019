...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & gold(X, Y, Q) & not movement_obs & not gold_being_collected(X, Y) <-
		- searching;
		+ collecting_res(X, Y);
		.send(scen4goldLeader, tell, gold_being_collected(X, Y));
		!! move_to_res(X, Y).
...
+! pickup_res : collecting_res(X, Y) & gold(X, Y, 0) <-
		- gold(X, Y, 0);
		.send(scen4goldLeader, tell, picked_gold(X, Y));
		- collecting;
		!! deposit_load.
...
+! deposit_res : carrying(0) <-
		- depositing;
		- collecting_res(X, Y);
		.send(scen4goldLeader, tell, collected_gold(X, Y));
		+ free.
