...
+! plan : true <-
		...
		+ max_speed(S);
		+ max_scan(R);
		actions.calc_searches_to_do(W, H, 2, 5, N);
		+ searches_to_do(N);
		+ reverse(1);
		+ free.
...
+! choose_task : diamond(X, Y, _) & not collecting_res(_, _) <-
		- free;
		! handle_res(X, Y, Q).
...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & diamond(X, Y, Q) & not movement_obs<-
		- searching;
		+ collecting_res(X, Y);
		!! move_to_res(X, Y).

+! handle_res(X, Y, Q) : not collecting_res(_, _) & not diamond(X, Y, _) & not movement_obs <-
		+ diamond(X, Y, Q);
		+ searching;
		+ collecting_res(X, Y);
		!! move_to_res(X, Y).
...
+! handle_other(T, Q, X, Y) : T == "Gold" <-
		- others(O);
		+ others(O - 1);
		.send(scenario3gold, tell, found_gold(X, Y, Q));
		! choose_action.
