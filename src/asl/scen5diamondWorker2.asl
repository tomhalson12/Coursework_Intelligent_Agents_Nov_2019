...
+! plan : true <-
		...
		+ searches_to_do(N);
		actions.calc_start_point(W, H, 6, 3, S_X, S_Y);
...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & diamond(X, Y, Q) & not movement_obs & not diamond_being_collected(X, Y) <-
		- searching;
		+ collecting_res(X, Y);
		.broadcast( tell, diamond_being_collected(X, Y));
		!! move_to_res(X, Y).
...
+! pickup_res : collecting_res(X, Y) & diamond(X, Y, 0) <-
		- diamond(X, Y, 0);
		- collecting;
		.broadcast( tell, picked_diamond(X, Y));
		!! deposit_load.
...
+! deposit_res : carrying(0) <-
		- collecting_res(X, Y);
		.broadcast( tell, collected_diamond(X, Y));
		- depositing;
		+ free.
