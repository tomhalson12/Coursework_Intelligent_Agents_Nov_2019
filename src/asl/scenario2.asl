...
+! handle_res(X, Y, Q) : not collecting_res(_, _) & gold(X, Y, Q) & not movement_obs<-
		- searching;
		+ collecting_res(X, Y);
		!! move_to_res(X, Y).

+! handle_res(X, Y, Q) : not collecting_res(_, _) & not gold(X, Y, _) & not movement_obs <-
		+ gold(X, Y, Q);
		- searching;
		+ collecting_res(X, Y);
		!! move_to_res(X, Y).
...
+! handle_other(T, Q, X, Y) : T == "Diamond" <-
		.send(scenario3diamond, tell, found_diamond(X, Y, Q));
		- others(O);
		+ others(O - 1);
		! choose_action.
